...
class ResAutoencoderXLSE(nn.Module):
    ...
    def __init__(self,
        ...
        # Skip connections
        self.skip_down1 = SkipConvDown(C1,1)
        self.skip_up1 = SkipConvUp(1,C1)

        self.skip_down2 = SkipConvDown(C2,1)
        self.skip_up2 = SkipConvUp(1,C2)

        self.skip_down3 = SkipConvDown(C3,1)
        self.skip_up3 = SkipConvUp(1,C3)

        self.skip_down4 = SkipConvDown(C4,1)
        self.skip_up4 = SkipConvUp(1,C4)

        # ---------- Decoder ----------
        # 32x32 -> 64x64, C5 -> C4
        self.up4 = nn.ConvTranspose2d(C5, C4, kernel_size=2, stride=2)
        dec4_blocks = [ResBlockSE(C5, C4)] + [ResBlockSE(C4, C4) for _ in range(b4-1)]
        self.dec4 = nn.Sequential(*dec4_blocks)

        # 64x64 -> 128x128, C4 -> C3
        self.up3 = nn.ConvTranspose2d(C4, C3, kernel_size=2, stride=2)
        dec3_blocks = [ResBlockSE(C4, C3)] + [ResBlockSE(C3, C3) for _ in range(b3-1)]
        self.dec3 = nn.Sequential(*dec3_blocks)

        # 128x128 -> 256x256, C3 -> C2
        self.up2 = nn.ConvTranspose2d(C3, C2, kernel_size=2, stride=2)
        dec2_blocks = [ResBlockSE(C3, C2)] + [ResBlockSE(C2, C2) for _ in range(b2-1)]
        self.dec2 = nn.Sequential(*dec2_blocks)

        # 256x256 -> 512x512, C2 -> C1
        self.up1 = nn.ConvTranspose2d(C2, C1, kernel_size=2, stride=2)
        dec1_blocks = [ResBlockSE(C2, C1)] + [ResBlockSE(C1, C1) for _ in range(b1-1)]
    ...
    def encode(self, x):
        skip1 = self.enc1(x)   # [B, C1, 512, 512]
        skip_down1 = self.skip_down1(skip1)
        x = self.pool1(skip1)  # 256

        skip2 = self.enc2(x)   # [B, C2, 256, 256]
        skip_down2 = self.skip_down2(skip2)
        x = self.pool2(skip2)  # 128

        skip3 = self.enc3(x)   # [B, C3, 128, 128]
        skip_down3 = self.skip_down3(skip3)
        x = self.pool3(skip3)  # 64

        skip4 = self.enc4(x)   # [B, C4, 64, 64]
        skip_down4 = self.skip_down4(skip4)
        x = self.pool4(skip4)  # 32

        x = self.bottleneck(x)  # [B, C5, 32, 32]
        return x, (skip_down1, skip_down2, skip_down3, skip_down4)

    def decode(self, z, skips):
        skip_down1, skip_down2, skip_down3, skip_down4 = skips

        skip_up4 = self.skip_up4(skip_down4)
        x = self.up4(z)    # [B, C4, 64, 64]
        x = torch.cat((x, skip_up4), dim=1)
        x = self.dec4(x)

        skip_up3 = self.skip_up3(skip_down3)
        x = self.up3(x)    # [B, C3, 128, 128]
        x = torch.cat((x, skip_up3), dim=1)
        x = self.dec3(x)

        skip_up2 = self.skip_up2(skip_down2)
        x = self.up2(x)    # [B, C2, 256, 256]
        x = torch.cat((x, skip_up2), dim=1)
        x = self.dec2(x)

        skip_up1 = self.skip_up1(skip_down1)
        x = self.up1(x)    # [B, C1, 512, 512]
        x = torch.cat((x, skip_up1), dim=1)
        x = self.dec1(x)
...
if __name__ == "__main__":
    ...
    model = ResAutoencoderXLSE(in_channels=1, base_channels=64).to(device)

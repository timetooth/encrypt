...
uv run train_resae.py \
  ...
  --checkpoint_path "./results/res_ae_se_sobel/checkpoints/best_model.pth" \
  --results ./results/res_ae_se_sobel \
  --positions AP \
  --batch_size 32 \
  --shuffle True \
  --num_workers 4 \
...
from loss import SobelReconstructionLoss
...
def train(train_dataloader,
    ...
    criterion = SobelReconstructionLoss(pix_weight=1.0, grad_weight=0.1, use_l1=False)
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for batch in train_dataloader:
            ...
            optimizer.zero_grad()
            outputs = model(imgs)
            loss, lpix, lgrad = criterion(outputs, imgs)
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar("Loss/pixel", lpix.item(), global_step)
            writer.add_scalar("Loss/grad",  lgrad.item(), global_step)
            global_step += 1

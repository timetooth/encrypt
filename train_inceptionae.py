...
uv run train_resae.py \
  ...
  --checkpoint_path "./results/inception_ae/checkpoints/best_model.pth" \
  --results ./results/inception_ae \
  --positions AP \
  --batch_size 32 \
  --shuffle True \
  --num_workers 4 \
...
from models.inception_ae import InceptionResAE
...
def get_args():
    ...
    parser.add_argument("--checkpoint_path", type=str, default="./results/inception_ae/checkpoints/last.pth",
                        help="Path to save model checkpoints (.pth)")
    parser.add_argument("--results", type=str, default="./results/inception_ae",
    ...
    parser.add_argument("--base_channels", type=int, default=64, help="Number of initial channels in inceptionAE")
...
def train(train_dataloader,
    ...
    model = InceptionResAE(in_channels=1, base_channels=base_channels).to(device)
